...
class ManualLocation implements LocationProvider {
  /**
   * Every registered listener, not just the most recent one â€” a fake that
...
/**
 * unlock() rejects on its first call, then succeeds on every call after â€”
...
/**
 * Throws synchronously on its first start() call â€” e.g. navigator.geolocation
 * being undefined on an insecure origin â€” then succeeds on every call after.
 */
...
class RecordingAudio implements AudioPlayer {
  ...
  stop() {
    this.playing = false;
    // Real players resolve their pending playback when cancelled â€” cancelling
...
describe('useTourPlayer', () => {
  ...
  it('keeps the manual selection current after it interrupts playback', async () => {
    ...
    // currentSegment â€” blanking the bar while seg-2 is actually playing.
    await act(async () => {
      result.current.playSegment('seg-2');
    });
  ...
  it('does not tear down the location listener or in-flight audio when the audio/location identity changes on re-render', async () => {
    ...
    // that alone â€” that requires depending on [audio, location] in the
  ...
  it('rolls back the start() guard when unlock() rejects, so a later start() still works', async () => {
    ...
    // The first call's unlock() rejects. The caller must see that failure â€”
    // not have it silently swallowed â€” so it can surface an error.
    await act(async () => {
  ...
  it('does not leave the drain queue deadlocked when location.start() throws after the intro was already enqueued', async () => {
    ...
    // design â€” some LocationProviders, e.g. SimulatedLocation, deliver their
    ...
    // state, `draining` never returns to false â€” nothing left in the test
    // (or the real app) would ever settle that promise â€” so a retried
  ...
  describe('outro', () => {
    ...
    it('enqueues the outro once every triggerable segment has played, and only once', async () => {
      ...
      // outro â€” it already played once.
      await act(async () => {
    ...
    it('does not fire the outro when a walker skips ahead without playing every stop', async () => {
      ...
      // seg-2 â€” a walker who skips ahead has not finished the tour.
      act(() => {
