...
export function TourMap({ tour, lastFix, playedIds, onSelectStop }: TourMapProps) {
  ...
  useEffect(() => {
    ...
    // React StrictMode's mount â†’ cleanup â†’ mount, the first instance's `load`
    ...
    try {
      ...
      // e.g. "An API access token is required to use Mapbox GL" â€” thrown
      // synchronously out of the constructor when the token is missing.
      setFailed(true);
      return;
    }

    // Asynchronous failures â€” a 404'd style, a token revoked mid-session â€”
